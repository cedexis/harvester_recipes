...
class NewRelicApdexTest {
    ...
    def void test_is_apdex_score_consistent_returns_false_when_scores_contain_different_values() {
        def application = new JsonSlurper().parseText(" {\"application_summary\": {\"response_time\": 0,\"apdex_target\": 0.5,\"apdex_score\": null,\"error_rate\": 0,\"throughput\": 0},\"health_status\": \"green\",\"id\": 4926656,\"platform\": \"promo.7regal.com[euams2]\",\"apdex_scores\": {\"A1\": 1.0,\"A2\": 0.5,\"AD\": 0.3,\"AE\": 0.75}}")
        def scores = get_recipe().invokeMethod("sortApdexScoresByValue", application)
        assert ! recipe.invokeMethod("isApdexScoresConsistent", scores) == true
    }

    @Test
    def void test_get_minimum_score_returns_median_score() {
        assert get_recipe().invokeMethod("getMinimumScore", application) == 1.0
    }

    @Test
    def void test_get_minimum_score_returns_median_score_with_random_values() {
        def application = new JsonSlurper().parseText("{\"application_summary\": {\"response_time\": 0,\"apdex_target\": 0.5,\"apdex_score\": null,\"error_rate\": 0,\"throughput\": 0},\"health_status\": \"green\",\"id\": 4926656,\"platform\": \"promo.7regal.com[euams2]\",\"apdex_scores\": {\"A1\": 0.0,\"A2\": 1.0,\"AD\": 0.50,\"AE\": 0.50}}")
        assert get_recipe().invokeMethod("getMinimumScore", application) == 0.5
    }
    ...
    def void test_apply_apdex_rules_removes_country_apdex_scores_and_adds_required_fields() {
        ...
        applications.each{ application ->
            ...
            assert application.apdex_minimum_score == 1.0
            assert application.apdex_consistent == true
        }
    }

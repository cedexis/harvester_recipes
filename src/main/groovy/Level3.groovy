        ...
        if ((config as Map).containsKey("level3_access_key") &&
            ...
            get_services(ag).each { service ->
                ...
                if (service.key == "CACHING") {
                    ...
                    total_monthly = response.summaryData.'*'.collect { [(it.name() as String): it.text()] }.sum()
...
//                    }
            ...
            def usage = total_monthly.volume as Float
            def bandwidth = total_monthly.mbps95 as Float

            return new JsonBuilder([
                    bandwidth: [unit: "Mbps", value: String.format("%.2f", bandwidth)],
                    usage: [unit: "GB", value: String.format("%.2f", usage)],
            ]).toPrettyString()
